...
import { toggleGazan67 } from "@/lib/admin-api";

const ALLOWED_EMAIL = "zanshugurov07@gmail.com";

export function Gazan67Button() {
  ...
  if (!isLoaded || email !== ALLOWED_EMAIL) return null;

  const handleClick = async () => {
    const next = !active;
    setBusy(true);
    setError(null);
    const result = await toggleGazan67(next);
    setBusy(false);
    if (result.ok) {
      setActive(next);
    } else {
  ...
  return (
    <div style={{ display: "inline-flex", flexDirection: "column", gap: 6, alignItems: "flex-start" }}>
      <button
        ...
        onClick={handleClick}
        disabled={busy}
        title={active ? "Выключить музыку Газана у всех" : "Врубить музыку Газана у всех"}
        style={{
          position: "relative",
          overflow: "hidden",
          minWidth: 120,
