...
export default function AnimeDetailPage() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (result.ok) { setAnime(result.data.data); setForm(toForm(result.data.data)); setNotice("Аниме обновлено"); setError(null); ui.toast({ tone: "success", title: "Аниме обновлено" }); }
  ...
  async function moderateComment(comment: AdminComment, action: "approve" | "reject") {
    ...
    if (result.ok) { const title = action === "approve" ? "Комментарий одобрен" : "Комментарий отклонён"; setNotice(title); ui.toast({ tone: "success", title }); load(); } else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); }
  }

  async function importEpisodes() {
    if (!anime?.id) return;
    const result = await importEpisodesForAnime(anime.id);
    if (result.ok) { setNotice("Импорт серий поставлен в очередь"); ui.toast({ tone: "success", title: "Импорт серий поставлен в очередь" }); } else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); }
  }
  ...
  async function uploadImage(kind: "poster" | "cover", event: ChangeEvent<HTMLInputElement>) {
    ...
    if (result.ok) { setAnime(result.data.data); setForm(toForm(result.data.data)); ui.toast({ tone: "success", title: kind === "poster" ? "Постер загружен" : "Баннер загружен" }); }
    else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); }
    setUploading(null);
  }
  ...
  async function removeImage(kind: "poster" | "cover") {
    ...
    if (result.ok) { setAnime(result.data.data); setForm(toForm(result.data.data)); ui.toast({ tone: "success", title: kind === "poster" ? "Постер удалён" : "Баннер удалён" }); }
    else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); }
    setUploading(null);
  }
  ...
  async function findBanners() {
    ...
    if (result.ok) { setBannerCandidates(result.data.data); ui.toast({ tone: "success", title: `Найдено баннеров: ${result.data.data.length}` }); }
    else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); }
  ...
  async function applyBanner(candidate: AdminBannerCandidate) {
    ...
    if (result.ok) { setAnime(result.data.data); setForm(toForm(result.data.data)); ui.toast({ tone: "success", title: "Баннер применён" }); }
    else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); }
  }
  ...
  async function toggleCoverLock() {
    ...
    if (result.ok) { setAnime(result.data.data); ui.toast({ tone: "success", title: result.data.data.cover_locked ? "Баннер заблокирован" : "Автообновление баннера включено" }); }
    else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); }
  }
  ...
  return <section className="page">
    <PageHeader kicker="карточка аниме" title={anime.title} description={`ID #${anime.id} · /${anime.slug} · управление описанием, тегами, сериями и комментариями.`} actions={<Link className="button secondary" href="/anime">← Назад</Link>} />
    ...
    <div className="grid" style={{ gridTemplateColumns: "minmax(280px, .75fr) minmax(320px, 1.25fr)" }}>
      <Card>
        <div style={{ display: "grid", gap: 16 }}>
          ...
          <div className="grid">
            <label className="button secondary" style={{ cursor: "pointer" }}>{uploading === "poster" ? "Загружаем…" : "Загрузить постер"}<input accept="image/jpeg,image/png,image/webp" hidden onChange={(event) => uploadImage("poster", event)} type="file" /></label>
            <button className="button secondary" disabled={!anime.poster_url || uploading === "poster"} onClick={() => removeImage("poster")} type="button">Удалить постер</button>
          </div>
          ...
          <div className="grid">
            <StatCard label="Рейтинг" value={`★ ${anime.rating ?? "—"}`} caption={`${formatNumber(anime.ratings_count)} оценок`} />
            <StatCard label="Серии" value={formatNumber(anime.episodes_count ?? anime.number_of_episodes)} caption="Связанных серий" />
            <StatCard label="Комментарии" value={formatNumber(anime.comments_count)} caption="Обсуждения" />
          </div>
      ...
      <Card>
        <form onSubmit={save} style={{ display: "grid", gap: 14 }}>
          <div className="grid"><label><span className="stat-card-label">Название</span><input className="input" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></label><label><span className="stat-card-label">Ссылка на постер</span><input className="input" value={form.poster_url} onChange={(e) => setForm({ ...form, poster_url: e.target.value })} /></label></div>
          <label><span className="stat-card-label">Описание</span><textarea className="input" rows={8} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></label>
          <div className="grid"><label><span className="stat-card-label">Статус</span><select className="select" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}><option value="planned">Запланировано</option><option value="ongoing">Выходит</option><option value="finished">Завершено</option><option value="paused">Приостановлено</option></select></label><label><span className="stat-card-label">Тип</span><select className="select" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}><option value="tv">Сериал</option><option value="movie">Фильм</option><option value="ova">OVA</option><option value="ona">ONA</option><option value="special">Спецвыпуск</option><option value="music">Музыкальное</option></select></label><label><span className="stat-card-label">Год</span><input className="input" value={form.year} onChange={(e) => setForm({ ...form, year: e.target.value })} /></label><label><span className="stat-card-label">Рейтинг</span><input className="input" value={form.rating} onChange={(e) => setForm({ ...form, rating: e.target.value })} /></label></div>
          ...
          <div><p className="stat-card-label">Теги</p><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{tags.map((tag) => <button className={`badge ${selectedTagSet.has(tag.id) ? "brand" : ""}`} key={tag.id} onClick={() => setForm({ ...form, tags: selectedTagSet.has(tag.id) ? form.tags.filter((id) => id !== tag.id) : [...form.tags, tag.id] })} type="button">{tag.name}</button>)}</div></div>
