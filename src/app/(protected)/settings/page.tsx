...
export default function SettingsPage() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.ok) { ui.toast({ tone: "success", title: "Настройки сохранены", message: `Применено изменений: ${payload.length}` }); load(); }
      else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); }
    } catch (error) {
      const message = error instanceof Error ? error.message : "Недопустимое значение настройки";
      setError(message); ui.toast({ tone: "error", title: "Ошибка", message });
    } finally { setSaving(false); }
  }

  if (loading) return <LoadingState label="Загружаем настройки…" />;
  if (error && settings.length === 0) return <ErrorState message={error} />;
  if (settings.length === 0) return <EmptyState title="Настройки не заданы" />;

  return <section className="page"><PageHeader kicker="управление системой" title="Настройки" description="Функции, правила модерации, загрузки, импорт и диагностика Aniyume." actions={<button className="button secondary" onClick={load} type="button">↻ Обновить</button>} />
    ...
    <div className="split-layout">
      <Card><form className="form-grid" onSubmit={save}><div className="toolbar"><div><p className="kicker">{activeGroup}</p><h2 style={{ margin: 0 }}>Настройки группы</h2></div><Badge tone={dirty.length ? "warning" : "success"}>Изменено: {dirty.length}</Badge></div>{groupSettings.map((setting) => <div key={setting.key} style={{ border: "1px solid var(--border)", borderRadius: 18, padding: 14, background: "rgba(255,255,255,.035)" }}><div className="toolbar" style={{ marginBottom: 10 }}><div><strong>{setting.key}</strong><p className="muted" style={{ margin: "4px 0 0" }}>{setting.description ?? "—"}</p></div><div style={{ display: "flex", gap: 6 }}><Badge>{setting.type}</Badge>{setting.is_public ? <Badge tone="brand">публичная</Badge> : null}</div></div>{setting.type === "boolean" ? <label className="pill" style={{ justifyContent: "flex-start" }}><input checked={Boolean(values[setting.key])} onChange={(e) => setValues({ ...values, [setting.key]: e.target.checked })} type="checkbox" /> Включено</label> : setting.type === "json" ? <textarea className="input" rows={6} value={String(values[setting.key] ?? "")} onChange={(e) => setValues({ ...values, [setting.key]: e.target.value })} /> : <input className="input" value={String(values[setting.key] ?? "")} onChange={(e) => setValues({ ...values, [setting.key]: e.target.value })} /> }<p className="muted" style={{ fontSize: 12 }}>Обновлено: {formatDate(setting.updated_at)}</p></div>)}<button className="button" disabled={saving || dirty.length === 0} type="submit">{saving ? "Сохраняем…" : `Сохранить изменения: ${dirty.length}`}</button></form></Card>
      <Card><p className="kicker">диагностика</p><h2 style={{ marginTop: 0 }}>Состояние окружения</h2>{diagnostics ? <div style={{ display: "grid", gap: 16 }}>{Object.entries(diagnostics).map(([group, entries]) => <div key={group}><h3 style={{ marginBottom: 8 }}>{group}</h3><div style={{ display: "grid", gap: 8 }}>{Object.entries(entries).map(([key, value]) => <div className="pill" key={key} style={{ justifyContent: "space-between", borderRadius: 14 }}><span>{key}</span><strong style={{ color: typeof value === "boolean" ? value ? "var(--success)" : "var(--danger)" : "var(--text)" }}>{String(value)}</strong></div>)}</div></div>)}</div> : <p className="muted">Диагностика недоступна.</p>}</Card>
    </div>
