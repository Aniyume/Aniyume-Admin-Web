...
export default function TagsPage() {
  ...
  async function createTag() { const name = await ui.prompt({ title: "Новый тег", placeholder: "Название тега", confirmLabel: "Создать" }); if (!name?.trim()) return; const result = await createAdminTag(name.trim()); if (result.ok) { setNotice("Tag created"); ui.toast({ tone: "success", title: "Tag created" }); load(); } else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); } }
  ...
  return <section className="page"><PageHeader kicker="taxonomy" title="Tags / Genres" description="Жанры и теги с количеством anime. Добавлены create/edit/delete actions; merge и bulk assign — следующий этап." actions={<button className="button" onClick={createTag} type="button">+ Новый тег</button>} />
