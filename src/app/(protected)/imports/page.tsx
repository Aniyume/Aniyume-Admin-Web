...
import { AdminImportsDashboard, getAdminImportsDashboard, importAllEpisodes, runAdminImport } from "@/lib/admin-api";
...
export default function ImportsPage() {
  ...
  const [running, setRunning] = useState<"initial" | "update" | "episodes" | "full" | null>(null);
  const ui = useUi();
  ...
  async function runFullUpdate() {
    const confirmed = await ui.confirm({ title: "Запустить full update", message: "Full update запустит обновление anime metadata, затем массовый импорт эпизодов. Баннеры улучшаются на странице конкретного anime через Find better/Apply, пока bulk banner endpoint не добавлен в API.", confirmLabel: "Запустить всё" });
    if (!confirmed) return;
    setRunning("full");
    ...
    ui.toast({ tone: "success", title: "Full update queued", message: `Anime log #${animeResult.data.data.id}; episodes queued ${episodesResult.data.data.queued_count ?? 0}` });
    load();
    setRunning(null);
  }
  ...
  return <section className="page"><PageHeader kicker="data operations" title="Imports" description="Запуск anime import, контроль очереди и диагностика. Если на хостинге import не идёт — почти всегда не запущен Laravel queue worker." actions={<button className="button secondary" onClick={load} type="button">↻ Refresh</button>} />
    ...
    <div className="grid" style={{ gridTemplateColumns: "minmax(320px, .9fr) minmax(320px, 1.1fr)" }}><Card><p className="kicker">run import</p><h2 style={{ marginTop: 0 }}>Запуск полного импорта</h2><p className="muted" style={{ lineHeight: 1.6 }}>Можно запускать anime metadata и episodes вместе. Кнопки ставят задачи в Redis queue, поэтому после старта можно закрыть вкладку — worker продолжит выполнение.</p><div style={{ display: "grid", gap: 12 }}><button className="button" disabled={!!running} onClick={runFullUpdate} type="button">{running === "full" ? "Starting…" : "Full update: anime + episodes"}</button><button className="button secondary" disabled={!!running} onClick={() => run("update")} type="button">{running === "update" ? "Starting…" : "Anime update only"}</button><button className="button secondary" disabled={!!running} onClick={runEpisodes} type="button">{running === "episodes" ? "Starting…" : "Episodes import only"}</button><button className="button secondary" disabled={!!running} onClick={() => run("initial")} type="button">{running === "initial" ? "Starting…" : "Initial anime import"}</button></div></Card>
    ...
    <Card><p className="kicker">best practice</p><h2 style={{ marginTop: 0 }}>Как лучше сделать импорт</h2><div className="grid"><div><h3>Anime metadata</h3><p className="muted">Обновляет карточки anime: title, poster, status, tags, rating и новые записи.</p></div><div><h3>Episodes</h3><p className="muted">Episodes import запускается отдельной задачей, пропускает существующие серии и не должен создавать дубли.</p></div><div><h3>Banners</h3><p className="muted">Для баннеров уже есть Find better на странице anime detail. Массовый auto-enrich появится после добавления bulk endpoint в API.</p></div><div><h3>Queue worker</h3><p className="muted">На production должен работать api-worker. Без него ImportLog останется running, потому что job поставлена, но никто её не выполняет.</p></div></div></Card>
  </section>;
}
