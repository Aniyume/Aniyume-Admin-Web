...
import { AdminContactMessage, PaginationMeta, deleteAdminContact, getAdminContactPhotoObjectUrl, getAdminContacts, updateAdminContactStatus } from "@/lib/admin-api";
...
export default function ContactsPage() {
  ...
  async function setStatus(contact: AdminContactMessage, status: string) { const note = ["resolved", "archived"].includes(status) ? await ui.prompt({ title: "Комментарий администратора", defaultValue: contact.admin_note ?? "", confirmLabel: "Сохранить" }) ?? undefined : undefined; const result = await updateAdminContactStatus(contact.id, status, note); if (result.ok) { ui.toast({ tone: "success", title: `Обращение #${contact.id} обновлено` }); setSelected(result.data.data); load(); } else { setError(result.message); ui.toast({ tone: "error", title: "Ошибка", message: result.message }); } }
  ...
  async function openPhoto(contact: AdminContactMessage) { const result = await getAdminContactPhotoObjectUrl(contact.id); if (result.ok) window.open(result.data, "_blank", "noreferrer"); else ui.toast({ tone: "error", title: "Ошибка", message: result.message }); }

  return <section className="page"><PageHeader kicker="обратная связь" title="Обращения" description="Сообщения пользователей: ошибки, идеи, отзывы и проблемы с контентом." />
    <Card><form className="filter-row" onSubmit={submit}><input className="input" value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} placeholder="Поиск по теме, тексту или email" /><select className="select" value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })}><option value="">Все статусы</option><option value="new">Новое</option><option value="reviewed">Просмотрено</option><option value="resolved">Решено</option><option value="archived">В архиве</option></select><select className="select" value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })}><option value="">Все категории</option><option value="bug">Ошибка</option><option value="idea">Идея</option><option value="feedback">Отзыв</option><option value="content">Контент</option><option value="account">Аккаунт</option><option value="other">Другое</option></select><button className="button secondary" type="submit">Фильтровать</button></form></Card>
